  ...
  describe('Tasks', () => {
    describe('methods', () => {
      ...
      it(`can't delete task from another owner`, () => {
        const fn = () => mockMethodCall('tasks.remove', taskId, { context: { userId: 'somebody-else-id' } });
        assert.throw(fn, /Access denied/);
        assert.equal(TasksCollection.find().count(), 1);
      });

      it('can change a status of a task', () => {
        const originalTask = TasksCollection.findOne(taskId);
        mockMethodCall('tasks.setIsChecked', taskId, !originalTask.isChecked, { context: { userId } })

        const updateTask = TasksCollection.findOne(taskId);
        assert.notEqual(updateTask.isChecked, originalTask.isChecked)
      })

      it('can insert new tasks', () => {
        const text = 'New Task';
        mockMethodCall('tasks.insert', text, { context: { userId } });
